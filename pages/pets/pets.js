...
const list_element = document.getElementById('pets-selection');
const btnRight = document.querySelector('.slider-front');
const currentPage = document.querySelector('.slider-current');
const btnLeft = document.querySelector('.slider-back');
const firstPage = document.getElementById('start');
const lastPage = document.getElementById('end');
const popUp = document.querySelector('.pop-up');
const popUpDiv = document.querySelector('.pop-up-data');


let current_page = 0;
let rows = screen.width >= 1280 ? 8 : screen.width >= 768 ? 6 : 3; 
...
function DisplayList(items, wrapper, rows_per_page, page){
  ...
  let randomArr = paginatedItems.sort(() => Math.random() - 0.5);
  ...
  for(let i = 0; i < paginatedItems.length; i++){
    ...
    for(let i = 0; i < paginatedItems.length; i++){      
      petContent[i].addEventListener('click', function(e){
        ...
        <div class="more-info-container">
          ...
          <ul>
            ...
            <li><strong>Inoculations:</strong> ${inoculations.join(', ')}</li>
            <li><strong>Diseases:</strong> ${diseases.join(', ')}</li>
            <li><strong>Parasites:</strong> ${parasites.join(', ')}</li>
          </ul>
        </div>`
      ...
      document.querySelector('.more-info-container').style.width = '100%';    
      })
    }
  }

DisplayList(pets, list_element, rows, current_page);
...
function deactivateLefttSlider(){
  btnLeft.setAttribute('disabled', '');
  firstPage.setAttribute('disabled', '');
  btnLeft.classList.add('slider-inactive');
  btnLeft.classList.remove('slider-active');
  firstPage.classList.add('slider-inactive');
  firstPage.classList.remove('slider-active');
}


function activateLeftSlider(){
  btnLeft.removeAttribute('disabled');
  firstPage.removeAttribute('disabled');
  btnLeft.classList.add('slider-active');
  btnLeft.classList.remove('slider-inactive');
  firstPage.classList.add('slider-active');
  firstPage.classList.remove('slider-inactive');
}

function activateRightSlider(){
  btnRight.removeAttribute('disabled');
  lastPage.removeAttribute('disabled');
  btnRight.classList.add('slider-active');
  btnRight.classList.remove('slider-inactive');
  lastPage.classList.add('slider-active');
  lastPage.classList.remove('slider-inactive');
}

function deactivateRighttSlider(){
  btnRight.setAttribute('disabled', '');
  lastPage.setAttribute('disabled', '');
  btnRight.classList.add('slider-inactive');
  btnRight.classList.remove('slider-active');
  lastPage.classList.add('slider-inactive');
  lastPage.classList.remove('slider-active');
...
firstPage.addEventListener('click', () => {
  ...
  DisplayList(pets, list_element, rows, current_page);
  currentPage.textContent = current_page + 1;
})

lastPage.addEventListener('click', () => {
  current_page = (pets.length / paginatedItems.length) - 1;

  activateLeftSlider();

  // INACTIVE LAST PAGE

  deactivateRighttSlider();
...
btnRight.addEventListener('click', () => {
  ...
  if(current_page >= (pets.length / paginatedItems.length) - 1){
    deactivateRighttSlider();
  } else {
    DisplayList(pets, list_element, rows, current_page);
  }
